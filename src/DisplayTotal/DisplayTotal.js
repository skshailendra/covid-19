...
const DisplayTotal = (props) => {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(casesTimeSeries) && casesTimeSeries.length > 0) {
      filterArray = casesTimeSeries.filter((item) =>
        item.date.includes(getCurrentMonth())
      );
      filterArray.map((item) => {
        ...
        item.dailyactive =
          item.dailyconfirmed - (item.dailyrecovered + item.dailydeceased);
        ...
        <div className={`display-total ${thememode}`}>
          {statewise && (
            <>
              ...
              <div className={`display-total__block  ${thememode}`}>
                <div className={`display-total__text active`}>{"Active"}</div>
                <div className={`display-total__count-block`}>
                  <div className="display-total__count">
                    <CountUp
                      start={200000}
              ...
              <div className={`display-total__block ${thememode}`}>
                <div className={`display-total__text deaths`}>{"Deaths"}</div>
                <div className={`display-total__count-block`}>
                  <div className="display-total__count">
                    <CountUp
                      start={0}
