  ...
  useEffect(() => {
    ...
    if (Array.isArray(casesTimeSeries) && casesTimeSeries.length > 0) {
      filterArray = casesTimeSeries.filter((item) =>
        item.date.includes(filterData.month)
      );
            ...
            <>
              ...
              <div className={`bar-description-abbr `}>
                <div className={`bar-abbr-status ${thememode}`}>
                  <span
                    className={`bar-abbr-status__confirmed bar-abbr-status__bar-abbr-color`}
                  ></span>
                  <span>Confirmed</span>
                </div>
                <div className={`bar-abbr-status ${thememode}`}>
                  <span
                    className={`bar-abbr-status__recovered bar-abbr-status__bar-abbr-color`}
                  ></span>
                  <span>Recovered</span>
                </div>
                <div className={`bar-abbr-status ${thememode}`}>
